...
serve(async (req) => {
  ...
  try {
    ...
    if (!token || token.length < 10) {
      logStep("Invalid token format - returning free tier");
      return new Response(JSON.stringify({ 
        subscribed: false,
        tier: "free",
        subscription_end: null 
    ...
    if (userError) {
      logStep("Auth error - returning free tier", { error: userError.message });
      return new Response(JSON.stringify({ 
        subscribed: false,
        tier: "free",
        subscription_end: null 
    ...
    await supabaseClient
      ...
      .update({ 
        ...
        storage_limit_mb: tier === "heritage" ? 20480 : tier === "premium" ? 10240 : 500
      })
      .eq("user_id", user.id);
