...
const translations: Record<string, {
  ...
  fr: {
    ...
    intro: "Merci de vous être inscrit(e) sur Family Garden ! Pour activer votre compte et commencer à préserver vos souvenirs, veuillez confirmer votre adresse email.",
    buttonText: "Confirmer mon adresse email",
    instruction: "Si le bouton ne fonctionne pas, copiez et collez ce lien dans votre navigateur :",
  ...
  en: {
    ...
    intro: "Thank you for signing up for Family Garden! To activate your account and start preserving your memories, please confirm your email address.",
    buttonText: "Confirm my email address",
    instruction: "If the button doesn't work, copy and paste this link into your browser:",
    expiry: "This link expires in 24 hours.",
    footer: "If you didn't create a Family Garden account, you can ignore this email.",
  },
  es: {
    subject: "Confirma tu dirección de correo — Family Garden",
    greeting: (name) => `Hola ${name},`,
    intro: "¡Gracias por registrarte en Family Garden! Para activar tu cuenta y comenzar a preservar tus recuerdos, confirma tu dirección de correo electrónico.",
    buttonText: "Confirmar mi correo",
    instruction: "Si el botón no funciona, copia y pega este enlace en tu navegador:",
  ...
  ko: {
    ...
    intro: "Family Garden에 가입해 주셔서 감사합니다! 계정을 활성화하고 추억을 보존하려면 이메일 주소를 확인해 주세요.",
    buttonText: "이메일 주소 확인",
    instruction: "버튼이 작동하지 않으면 아래 링크를 브라우저에 복사하여 붙여넣으세요:",
  ...
  zh: {
    ...
    intro: "感谢您注册 Family Garden！要激活您的账户并开始保存回忆，请确认您的邮箱地址。",
    buttonText: "确认我的邮箱地址",
    instruction: "如果按钮无效，请将以下链接复制粘贴到浏览器中：",
...
function buildEmailHtml(t: ReturnType<typeof getTranslation>, confirmationUrl: string, displayName: string): string {
  return `<!DOCTYPE html>
<html>
        ...
        <tr><td style="padding:36px 40px;">
          ...
          <table width="100%" cellpadding="0" cellspacing="0">
            <tr><td align="center" style="padding:8px 0 28px;">
              <a href="${confirmationUrl}" style="display:inline-block;background:linear-gradient(135deg,#2D5A3D,#4A7C59);color:#ffffff;text-decoration:none;padding:14px 36px;border-radius:10px;font-size:15px;font-weight:600;letter-spacing:0.3px;">${t.buttonText}</a>
            </td></tr>
          </table>
          <p style="margin:0 0 8px;color:#888;font-size:13px;">${t.instruction}</p>
          <p style="margin:0 0 20px;word-break:break-all;color:#2D5A3D;font-size:12px;">${confirmationUrl}</p>
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const { email, confirmationUrl, displayName, locale } = await req.json() as ConfirmationEmailRequest;
